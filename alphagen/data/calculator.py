...
class TensorAlphaCalculator(AlphaCalculator):
    ...
    def make_ensemble_alpha(self, exprs: Sequence[Expression], weights: Sequence[float]) -> Tensor:
        print("script", f"表达式列表: {exprs}")
        print("script", f"权重: {weights}")
        if not exprs or weights is None or len(weights) == 0:
            raise ValueError("表达式列表或权重为空，无法生成因子。")

        n = len(exprs)
        factors = []
        for i in range(n):
            alpha_tensor = self.evaluate_alpha(exprs[i])
            print("script", f"表达式 {i} 的 alpha tensor: {alpha_tensor.size()}")
            if alpha_tensor.numel() == 0:
                raise ValueError(f"表达式 {i} 返回了一个空的 tensor。")
            weighted_alpha = alpha_tensor * weights[i]
            if weighted_alpha.numel() == 0:
                raise ValueError(f"加权后的表达式 {i} 返回了一个空的 tensor。")
            factors.append(weighted_alpha)

        if not factors:
            raise ValueError("没有生成有效的 factor 进行组合。")
